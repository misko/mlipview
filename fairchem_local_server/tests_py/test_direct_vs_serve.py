...
async def test_single_point_relax_md_equivalence(molecules, ray_serve_app, mol_name):
    ...
    dyn = Langevin(atoms, 1.0 * units.fs, temperature_K=TEMP_K, friction=0.02, logfile=None)
...
@pytest.mark.asyncio
async def test_batched_requests_equivalence(molecules, ray_serve_app):
    # Send 3 relax requests together ensure batching kicked in (cannot easily introspect batch size without instrumentation, so rely on timing and success)
    payloads = []
    for name in ['water','benzene','roy']:
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        r = await client.post(f'{ray_serve_app}/serve/relax/batch', json=payloads)
        assert r.status_code == 200
        out = r.json()
        assert isinstance(out, list) and len(out) == 3
        # Validate energies deterministic vs standalone calculation for first step baseline
        for name, item in zip(['water','benzene','roy'], out):
            assert 'initial_energy' in item
            assert 'final_energy' in item
            # baseline single-point
            atoms = molecules[name].copy()
            Z = list(atoms.get_atomic_numbers())
            atoms.calc = _server._CALCULATOR
            e0 = float(atoms.get_potential_energy())
            assert math.isclose(e0, float(item['initial_energy']), rel_tol=1e-6, abs_tol=1e-4)
