...
export function createManipulationService(molState, { bondService } = {}) {
  ...
  // Debug instrumentation removed for production slimness.
  ...
  function beginDrag(intersector) {
    ...
    if (molState.selection.kind !== 'atom') return false;
    const atomIndex = molState.selection.data.index;
    const pos = getAtomPosition(atomIndex);
    const planeNormal = { x:0, y:0, z:1 }; // default; caller can override later via setDragPlane
  ...
  function rotateBond(deltaAngle, sideOverride) {
    if (molState.selection.kind !== 'bond') return false;
