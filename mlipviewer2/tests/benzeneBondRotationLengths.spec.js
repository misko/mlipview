...
// Jest in ESM mode doesn't define __dirname; derive it from current working directory + this test's relative path.
const testDir = path.join(process.cwd(), 'mlipviewer2', 'tests');
...
describe('Benzene bond rotation keeps reasonable bond lengths (with cell + ghosts)', () => {
  test('rotating a selected C-C bond by various angles maintains sane lengths', () => {
    const benzenePath = path.join(process.cwd(), 'mlipviewer2', 'public', 'molecules', 'benzene.xyz');
