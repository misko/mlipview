...
function symbolToZ(s) {
  return SYMBOL_TO_Z[s] || 0;
}
...
async function buildReferenceTrace(page) {
  ...
  const energies = await page.evaluate(
    async ({ atomic_numbers, coords, steps }) => {
...
test('ROY multi-step relax parity (20 steps) vs dynamic backend reference', async ({
  page,
  baseURL,
}) => {
