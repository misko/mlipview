...
test('Idle drag sends USER_INTERACTION and updates forces/energy', async ({ page, baseURL }) => {
  ...
  // Warm WS and seed initial energy for plot
  ...
  const results = await page.evaluate(async () => {
    ...
    for (let i = 0; i < els.length; i++) {
      ...
      if (isC) { idx = i; break; }
      if (!isH) idx = i; // fallback to any non-hydrogen if no carbon found
    }
    ...
    // Count outgoing USER_INTERACTION messages by wrapping the client API
    const origUserInteraction = ws.userInteraction.bind(ws);
    ws.userInteraction = (arg) => {
      try {
        sentUI++;
        if (arg && Array.isArray(arg.positions)) sentUIWithPos++;
      } catch { }
      return origUserInteraction(arg);
    };
    ...
    // Drag the selected atom around while idle; each update triggers an idle compute
    ...
    for (let i = 0; i < 20; i++) {
      ...
      // small delay to allow USER_INTERACTION + energy return
      // also request deterministic idle compute to speed up
      try { await window.viewerApi.requestSimpleCalculateNow(); } catch { }
      await new Promise(r => setTimeout(r, 30));
      // Note forces may update asynchronously after energy arrival; sample a few times
      try {
    ...
    // allow a brief settle for last energy/forces
    const t0 = Date.now();
    ...
    ws.userInteraction = origUserInteraction;
    return { sentUI, sentUIWithPos, energyFrames, forcesUpdated, energyTicksDelta: energyLen1 - energyLen0 };
  });

  // Assertions: we should have sent many USER_INTERACTION updates with positions,
  // received multiple energy frames, forces updated at least once, and energy plot advanced.
