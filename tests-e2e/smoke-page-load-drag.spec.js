...
test('page load, drag atom, energy tick', async ({ page, baseURL }) => {
  ...
  await page.evaluate((b)=>{ window.__BASE_LEN__ = b|0; }, baseLen);
  ...
  await page.evaluate(() => {
    const api = window.viewerApi; const p = api.state.positions[0];
    const intersector = () => ({ x: p.x, y: p.y, z: p.z });
    api.manipulation.beginDrag(intersector, { planePoint:{ x:p.x,y:p.y,z:p.z }, planeNormal:{ x:1,y:0,z:0 }, source:'e2e' });
  });
  await page.evaluate(() => {
    const api = window.viewerApi; const p = api.state.positions[0];
  ...
  const changed = Math.abs(afterPos[0]-beforePos[0]) + Math.abs(afterPos[1]-beforePos[1]) + Math.abs(afterPos[2]-beforePos[2]);
  ...
  const maybeLen = await waitFor(page, () => {
    const len = window.viewerApi?.debugEnergySeriesLength?.() || 0;
    return len >= 1 ? len : 0;
  }, { timeout: 30000 }).catch(() => 0);
