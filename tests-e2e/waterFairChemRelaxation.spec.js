...
async function serverReachable(page) {
  try {
    const resp = await page.request.post('http://127.0.0.1:8000/simple_calculate', {
      data: { atomic_numbers: [1], coordinates: [[0, 0, 0]] },
    });
    return resp.ok();
  } catch {
    return false;
...
test('water FairChem BFGS 20-step first/last parity', async ({ page }) => {
  ...
  for (let i = 0; i < 20; i++) {
    ...
    await page.waitForFunction(
      (expected) => (window.__RELAX_TRACE || []).length >= expected,
      i + 2
    );
  }
