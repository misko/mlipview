  ...
  test('USER_INTERACTION sent and first energy plotted', async ({ page, baseURL }) => {
    ...
    page.on('websocket', (ws) => {
      ...
      ws.on('framesent', (data) => {
        wsEvents.push({ type: 'send', size: data?.length || 0 });
      });
      ws.on('framereceived', (data) => {
        wsEvents.push({ type: 'recv', size: data?.length || 0 });
      });
    ...
    await page.waitForFunction(
      () =>
        !!(
          window.__fairchem_ws__ &&
          window.__fairchem_ws__.getState &&
          window.__fairchem_ws__.getState().connected
        ),
      { timeout: 10000 }
    );
