...
async function ensureWater(page){
  ...
  await page.evaluate(async ()=>{ window.__FORCE_SYNC_REMOTE__=true; window.__MLIPVIEW_SERVER='http://localhost:8000'; try { await window.viewerApi?.baselineEnergy?.(); } catch(e){ console.log('baselineEnergy error', e); } });
  // Fallback polling: manually invoke ff.computeForces sync if energy not set yet
  await page.waitForFunction(()=>{
    const have = typeof window.viewerApi?.state?.dynamics?.energy === 'number' && isFinite(window.viewerApi.state.dynamics.energy);
    if(!have){ try { window.viewerApi?.ff?.computeForces?.({ sync:true }); } catch{} }
    return typeof window.viewerApi?.state?.dynamics?.energy === 'number' && isFinite(window.viewerApi.state.dynamics?.energy);
  }, null, { timeout: 20000 });
...
test('multi-step relax parity (20 steps) vs reference BFGS trace', async ({ page }) => {
  ...
  await page.addInitScript(()=>{ window.__FORCE_SYNC_REMOTE__=true; });
  await page.goto('/');
  ...
  console.log('RELAX20_PARITY', JSON.stringify({ initialDiff: absDiffs[0], maxDiff: Math.max(...absDiffs), rms, energies, ref: ref.slice(0,energies.length) }));
  // Persist artifact
  await page.context()._fsWrite?.(); // no-op placeholder if custom hook exists
  try {
    const artifact = { steps: STEPS, maxAbs: Math.max(...absDiffs), rms, perStepDiffs: absDiffs, energies, reference: ref.slice(0, energies.length), timestamp: new Date().toISOString() };
    ...
    fsMod.writeFileSync('test-results/waterMultiStepRelaxParity.json', JSON.stringify(artifact, null, 2));
  } catch(e) {
