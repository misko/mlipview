...
beforeAll(()=>{
  global.BABYLON = {
    Engine: function(canvas){ this._canvas = canvas; this.runRenderLoop = (fn)=>{ /* noop in tests */ }; this.stopRenderLoop = ()=>{}; },
    Scene: function(){ this.onPointerObservable = { _isNative:true, add:()=>{} }; this.clearColor = { r:1,g:1,b:1,a:1 }; this.onBeforeRenderObservable = { add: ()=>{} }; this.pick = ()=>({ hit:false }); this.createPickingRay = ()=>({ origin:{x:0,y:0,z:-5}, direction:{x:0,y:0,z:1} }); },
    ArcRotateCamera: function(){ this.alpha=1; this.beta=1; this.radius=10; this.position={x:0,y:0,z:-10}; this.attachControl=jest.fn(); this.detachControl=jest.fn(); this.inputs={ removeByType: jest.fn(), remove: jest.fn(), attached:{ pointers:{} } }; },
...
describe.skip('viewer wiring installs touch controls', () => {
  test('touchControls flag set after init', async () => {
    ...
    // minimal state
    const elements = [{ symbol:'H' }]; const positions = [{ x:0,y:0,z:0 }]; const bonds = [];
