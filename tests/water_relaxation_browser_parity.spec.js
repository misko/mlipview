...
async function callRelax(calculator) {
  ...
  const body = { atomic_numbers, coordinates, steps: 20, calculator };
  const resp = await fetch('http://127.0.0.1:8000/serve/relax', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(body),
  });
  if (!resp.ok) throw new Error('relax failed ' + resp.status);
...
async function setupViewer() {
  ...
  return await mod.initNewViewer(canvas, {
    ...
    positions: [
      { x: 0, y: 0, z: 0 },
      { x: 0.9575, y: 0, z: 0 },
      { x: -0.2399872, y: 0.92662721, z: 0 },
    ],
    bonds: [],
  });
}

async function performBrowserSteps(api, n = 20) {
  for (let i = 0; i < n; i++) {
    await api.relaxStep();
  }
  return window.__RELAX_TRACE || [];
}

['uma'].forEach((calc) => {
  // relaxStep currently hardcodes UMA backend calculator
  describe(`browser 20-step parity ${calc}`, () => {
    test('first/last energy matches server /relax aggregate call within tolerance', async () => {
      ...
      if (!up) {
        console.warn('[browser parity] server not reachable; skipping');
        return;
      }
      ...
      const initResp = await fetch('http://127.0.0.1:8000/serve/simple', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
