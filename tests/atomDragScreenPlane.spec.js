...
describe('camera-aligned drag plane', () => {
  function setup() {
    ...
    const selection = {
      ...
      clickBond: () => {},
      clear: () => {
        st.selection = { kind: null, data: null };
        st.markSelectionChanged();
      },
    };
    ...
    const scene = {
      ...
      pointerY: 100,
      pick() {
        return { hit: true, pickedMesh: {}, thinInstanceIndex: 0 };
      },
      onPointerObservable: {
        add(fn) {
          this._cb = fn;
      ...
      createPickingRay(x, y) {
        const r = makeRay(camera.position, x, y, viewport);
    ...
    const nx = atomPos.x - camPos.x,
      ny = atomPos.y - camPos.y,
      nz = atomPos.z - camPos.z;
    const len = Math.hypot(nx, ny, nz) || 1;
    const planeNormal = { x: nx / len, y: ny / len, z: nz / len };
    const planePoint = { ...atomPos };
    ...
    const intersector = (pp, pn) => {
      ...
      const vx = pp.x - ray.origin.x,
        vy = pp.y - ray.origin.y,
        vz = pp.z - ray.origin.z;
  ...
  test('plane normal aligns with camera->atom and atom stays under pointer for X move', () => {
    ...
    expect(
      Math.hypot(dragState.grabOffset.x, dragState.grabOffset.y, dragState.grabOffset.z)
    ).toBeLessThan(1e-6);
    ...
    manip.updateDrag((pp, pn) => {
      ...
      const ray = {
        origin: { x: 0, y: 0, z: -10 },
        direction: { x: (scene.pointerX - 100) / 100, y: -(scene.pointerY - 100) / 100, z: 1 },
      };
      const dlen = Math.hypot(ray.direction.x, ray.direction.y, ray.direction.z);
      ray.direction.x /= dlen;
      ray.direction.y /= dlen;
      ray.direction.z /= dlen;
      const denom = pn.x * ray.direction.x + pn.y * ray.direction.y + pn.z * ray.direction.z;
      const vx = pp.x - ray.origin.x,
        vy = pp.y - ray.origin.y,
        vz = pp.z - ray.origin.z;
