...
describe('LJ MD water', () => {
  test('5-step LJ MD produces finite positions', async () => {
    ...
    const body = { atomic_numbers:[8,1,1], coordinates:[[0,0,0],[0.95,0,0],[-0.24,0.93,0]], steps:5, temperature:298, timestep_fs:1.0, friction:0.02, calculator:'lj' };
    const r = await post('/md', body);
    expect(r.ok).toBe(true);
    const resp = r.json;
    expect(Array.isArray(resp.positions)).toBe(true);
    for(const p of resp.positions){ expect(p.length).toBe(3); for(const v of p){ expect(Number.isFinite(v)).toBe(true); } }
