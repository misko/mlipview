...
if (!global.BABYLON) {
  const BABYLON = {
    ...
    Scene: class Scene {
      ...
      createPickingRay() {
        return { origin: new BABYLON.Vector3(0, 0, 0), direction: new BABYLON.Vector3(0, 0, 1) };
      }
      render() {
        /* noop */
      }
    ...
    Matrix: class Matrix {
      ...
      clone() {
        const c = new BABYLON.Matrix();
        c.m.set(this.m);
        return c;
      }
      copyToArray(arr, off) {
        for (let i = 0; i < 16; i++) arr[off + i] = this.m[i] || 0;
      }
      static Identity() {
    ...
    MeshBuilder: {
      CreateSphere(name, opts, scene) {
        return new BABYLON.Mesh(name);
      },
      CreateCylinder(name, opts, scene) {
        return new BABYLON.Mesh(name);
      },
      CreateLines(name, opts, scene) {
        const m = new BABYLON.Mesh(name);
        m.dispose = function () {};
        return m;
      },
    },
    ...
    global.__logFlush = () => {
      ...
      if (summary.length) {
        ...
        for (const s of summary) {
          orig.log(`  [${s.kind}] x${s.occurrences}: ${s.preview}`);
        }
      }
    };
    process.on('exit', () => {
