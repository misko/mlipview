...
class Vector3 {
  ...
  rotateByQuaternionToRef(q, out) {
    // simplified rotation assuming unit quaternion
    const x = this.x,
      y = this.y,
      z = this.z;
    const qx = q.x || 0,
...
describe('vr drag stability', () => {
  test('atom drag under jitter yields smooth path', () => {
    ...
    const controller = {
      ...
      inputSource: {},
      getForwardRay() {
        return new Ray(new Vector3(0, 0, 0), new Vector3(0, 0, -1));
      },
    };
    // Inject controller
    ...
    function intersector(point, normal) {
      const ray = jitterFn();
      const nx = 0,
        ny = 1,
        nz = 0;
      const rd = ray.direction;
      const denom = rd.x * nx + rd.y * ny + rd.z * nz || 1e-6;
      const t = -(ray.origin.y - point.y) / (rd.y || 1e-6);
      return { x: ray.origin.x + rd.x * t, y: point.y, z: ray.origin.z + rd.z * t };
    }
    // Start drag
