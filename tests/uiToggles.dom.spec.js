...
jest.mock('../public/render/scene.js', () => ({ createScene: async () => ({ engine:{ runRenderLoop:(fn)=>{} }, scene:{ meshes:[], render:()=>{}, onPointerObservable:{ add(){} } }, camera:{} }) }));

beforeAll(()=>{
  if(!global.BABYLON){
    global.BABYLON = {
      TransformNode: class {},
      MeshBuilder:{ CreateSphere:()=>({ setEnabled(){} }), CreateCylinder:()=>({ setEnabled(){}, thinInstanceSetBuffer(){}, material:{}, _buffers:{} }), CreateLines:()=>({ dispose(){} }) },
      StandardMaterial: class {},
      Color3: class {},
      Vector3: class { constructor(x=0,y=0,z=0){ this.x=x; this.y=y; this.z=z; } },
      Quaternion: class { static Identity(){ return {}; } },
...
global.fetch = async (url, opts) => {
  ...
  if(/md$/.test(url)){
    const body = JSON.stringify({ positions:[[0,0,0]], velocities:[[0,0,0]], forces:[[0,0,0]], final_energy:-1.0, temperature:300 });
    return { ok:true, status:200, json: async ()=> JSON.parse(body), text: async ()=> body };
  }
  if(/relax$/.test(url)){
    const body = JSON.stringify({ positions:[[0,0,0]], forces:[[0,0,0]], final_energy:-1.0 });
    return { ok:true, status:200, json: async ()=> JSON.parse(body), text: async ()=> body };
  }
  if(/health/.test(url)){
...
test('connect toggles: forces toggle works', async () => {
  ...
  const api = await mod.initNewViewer(document.getElementById('viewer'), { elements:['O'], positions:[{x:0,y:0,z:0}], bonds:[] });
...
test('MD auto-run on page load sets status/button', async () => {
  ...
  const { buildDesktopPanel } = await import('../public/ui/desktopPanel.js');
  const mod = await import('../public/index.js');
  const api = await mod.initNewViewer(document.getElementById('viewer'), { elements:[{Z:8}], positions:[{x:0,y:0,z:0}], bonds:[] });
  buildDesktopPanel({ attachTo: document.getElementById('app') });
  ...
  for(let k=0;k<15 && btn.textContent!=='stop';k++) await new Promise(r=>setTimeout(r,10));
