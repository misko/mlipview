...
describe('UI: PBC enabled, breaking bond does not leave origin artifact', () => {
  test('no stray ghost cylinder/sphere at origin after bond disappears', () => {
    ...
    const bonds = createBondService(state);
    ...
    for (const g of view._internals.ghostBondGroups.values()) {
      ghostInst += g.mats.length;
      anyVisible = anyVisible || g.master.isVisible;
    }
    for (const g of view._internals.ghostAtomGroups.values()) {
      /* atom ghosts may remain */ if (g.mats.length === 0)
        anyVisible = anyVisible || g.master.isVisible;
    }
