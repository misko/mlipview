...
if(!B.GUI) B.GUI = {};
if(!B.GUI.Control){ B.GUI.Control = { HORIZONTAL_ALIGNMENT_CENTER: 0 }; }
if(!B.GUI.AdvancedDynamicTexture){
  B.GUI.AdvancedDynamicTexture = class { constructor(){ this._rootContainer={ children:[] }; }
    static CreateForMesh(_mesh, _w, _h){ return new B.GUI.AdvancedDynamicTexture(); }
    addControl(c){ this._rootContainer.children.push(c); }
  };
}
if(!B.GUI.Button){
  B.GUI.Button = class {
    static CreateSimpleButton(id, text){
      const obs = ()=>({ add:()=>{} });
      return { id, text, width:'', height:'', color:'', thickness:0, background:'', fontSize:0,
...
if(!B.GUI.Rectangle){
  B.GUI.Rectangle = class { constructor(){ this.children=[]; this.thickness=0; this.cornerRadius=0; this.background=''; }
    addControl(c){ this.children.push(c); }
  };
}
if(!B.GUI.StackPanel){
  B.GUI.StackPanel = class { constructor(){ this.isVertical=false; this.children=[]; }
    addControl(c){ this.children.push(c); }
  };
}
if(!B.GUI.Image){
  B.GUI.Image = class { constructor(id, source){ this.id=id; this.source=source; this.width=''; this.height=''; this.stretch=0; }
    markAsDirty(){}
  };
  B.GUI.Image.STRETCH_UNIFORM = 0;
}
if(!B.GUI.TextBlock){
  B.GUI.TextBlock = class { constructor(id,text){ this.id=id; this.text=text; this.color=''; this.fontSize=0; }
  };
}

// Scene & camera stub (prefer existing Scene if defined)
const scene = new (B.Scene || function(){ this.onBeforeRenderObservable={ _l:[], add(fn){ this._l.push(fn); return fn; } }; })();
scene.activeCamera = {
  fov: Math.PI/2,
...
if(!scene.onBeforeRenderObservable){
  scene.onBeforeRenderObservable = { _l:[], add(fn){ this._l.push(fn); return fn; }, remove(fn){ const i=this._l.indexOf(fn); if(i>=0) this._l.splice(i,1); } };
}
