...
describe('cross-image ghost bonds are generated', () => {
  test('augmented bond calculator yields at least one ghost bond', () => {
    const st = createMoleculeState({
      elements:['C','C'],
      positions:[ {x:0.9,y:0,z:0}, {x:2.1,y:0,z:0} ],
      bonds:[],
      cell:{ a:{x:3,y:0,z:0}, b:{x:0,y:3,z:0}, c:{x:0,y:0,z:3}, enabled:true, originOffset:{x:0,y:0,z:0} }
    });
    ...
    const baseAtoms = st.positions.map((p,i)=>({ element: st.elements[i], pos:[p.x,p.y,p.z] }));
    // Rebuild ghosts uses shifts including +/- x; ghost of atom 0 at +3 should be near atom 1 at 2.1 (distance 0.9)
    const aug = [ ...baseAtoms, ...baseAtoms.map(a=>({ element:a.element,pos:[a.pos[0]+3,a.pos[1],a.pos[2]] })) ];
