...
function createElasticCellProvider(targetLen=10, k=0.02){
  return {
    async compute({ cell }){
      ...
      return { energy:0, forces:[], stress:{ tensor:{ xx:sxx, yy:syy, zz:szz, xy:0,xz:0,yz:0 }, voigt:[sxx,syy,szz,0,0,0] } };
    }
  };
}
...
describe('cell optimizer', () => {
  test('reduces cell lengths toward target isotropically', async () => {
    const st = createSimulationState({ Z:[6], positions:[[0,0,0]], cell:[[12,0,0],[0,12,0],[0,0,12]] });
    const provider = createElasticCellProvider(10, 0.05);
    const opt = createCellOptimizer(st, { forceProvider: provider, settings:{ isotropic:true, cellGamma:0.5, stressTol:1e-3, maxSteps:50, maxStrainStep:0.2 } });
  ...
  test('no-op when provider returns no stress', async () => {
    const st = createSimulationState({ Z:[6], positions:[[0,0,0]], cell:[[11,0,0],[0,11,0],[0,0,11]] });
    const provider = { async compute(){ return { energy:0, forces:[], stress:null }; } };
