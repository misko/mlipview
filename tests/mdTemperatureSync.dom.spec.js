...
async function setup(){
  ...
  const ws = getWS(); const sent = []; ws.setTestHook(m=> sent.push(m));
  ...
  return { api, ws, sent, restore: ()=>{ global.WebSocket = origWS; } };
}
...
describe('Temperature slider sync', () => {
  test('initial load uses 1500K and mdStep payload matches', async () => {
    ...
    ws.injectTestResult({ positions: api.state.positions.map(p=>[p.x,p.y,p.z]), forces: [], energy: -1.0, temperature: window.__MLIP_TARGET_TEMPERATURE });
    await p;
  ...
  test('moving slider updates __MLIP_TARGET_TEMPERATURE and mdStep payload follows', async () => {
    ...
    ws.injectTestResult({ positions: api.state.positions.map(p=>[p.x,p.y,p.z]), forces: [], energy: -1.0, temperature: target });
    await p;
    const startMsg = sent.find(m=> m.simulationParams && typeof m.simulationParams.temperature === 'number');
    expect(startMsg).toBeTruthy();
