...
if(!B.GUI) B.GUI={}; if(!B.GUI.Control) B.GUI.Control={ HORIZONTAL_ALIGNMENT_CENTER:0, HORIZONTAL_ALIGNMENT_LEFT:1 };
...
if(!B.GUI.TextBlock) B.GUI.TextBlock = class { constructor(id,text){ this.id=id; this.text=text; this.color=''; } };
...
describe('XR HUD energy depth scaling', () => {
	test('topEnergyDistanceMult increases z without altering Y band drastically', () => {
		const scene1 = makeScene(); if(window.__XR_HUD_FALLBACK){ delete window.__XR_HUD_FALLBACK; delete window.__XR_HUD_FALLBACK_TOP; }
		const res1 = ensureWorldHUD({ scene:scene1, topEnergyDistanceMult:1 }); scene1.onBeforeRenderObservable._l.forEach(fn=>fn());
		if(window.__XR_HUD_FALLBACK){ delete window.__XR_HUD_FALLBACK; delete window.__XR_HUD_FALLBACK_TOP; }
		const scene2 = makeScene(); const res2 = ensureWorldHUD({ scene:scene2, topEnergyDistanceMult:3 }); scene2.onBeforeRenderObservable._l.forEach(fn=>fn());
		...
		expect(Math.abs(res2.planeTop.position.y)).toBeLessThan(Math.abs(res1.planeTop.position.y) * 1.2 + 1e-6);
	});
});
