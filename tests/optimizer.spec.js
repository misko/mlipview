  ...
  return {
    ...
    async compute({ positions }){
      ...
      for(const p of positions){
        const [x,y,z]=p; energy += 0.5*k*(x*x+y*y+z*z);
...
describe('optimizer core', () => {
  test('steepest descent reduces distance to origin', async () => {
    const st = createSimulationState({ Z:[6,6], positions:[[2,0,0],[0,2,0]] });
    const provider = createHarmonicProvider(1);
  const opt = createOptimizer(st, { forceProvider: provider, settings:{ algorithm:'sd', maxSteps:200, gamma:0.25, forceTol:5e-4 } });
  ...
  test('conjugate gradient converges on harmonic', async () => {
    const st = createSimulationState({ Z:[6,6], positions:[[1.5,0,0],[0,1.5,0]] });
    const provider = createHarmonicProvider(1);
    const opt = createOptimizer(st, { forceProvider: provider, settings:{ algorithm:'cg', maxSteps:80, gamma:0.3, forceTol:1e-5, cgReset:1000 } });
    const result = await opt.runUntil();
    expect(result.converged).toBe(true);
    // near origin
