...
const CC_MIN = 0.9;
const CC_MAX = 2.0; // anything longer is suspicious for a single bond in ring after rotation
const CH_MIN = 0.7;
const CH_MAX = 1.5;
...
function classifyBonds(state) {
  ...
  for (const b of state.bonds) {
    ...
    const e1 = state.elements[b.i];
    const e2 = state.elements[b.j];
...
describe('Benzene bond rotation keeps reasonable bond lengths (with cell + ghosts)', () => {
  test('rotating a selected C-C bond by various angles maintains sane lengths', () => {
    ...
    for (const deg of anglesDeg) {
      ...
      const ghostGroupBondCount = Array.from(view._internals.ghostBondGroups.values()).reduce(
        (s, g) => s + g.mats.length,
        0
      );
      ...
      for (const b of cc) {
        if (!(b.len >= CC_MIN && b.len <= CC_MAX)) {
          throw new Error(
            `C-C bond length out of range after ${deg}deg: ${b.i}-${b.j} len=${b.len.toFixed(3)}`
          );
        }
      }
      ...
      for (const b of ch) {
        if (!(b.len >= CH_MIN && b.len <= CH_MAX)) {
          throw new Error(
            `C-H bond length out of range after ${deg}deg: ${b.i}-${b.j} len=${b.len.toFixed(3)}`
          );
        }
      }
    }
