...
describe('VR drag excludes MD updates for held atom', () => {
  beforeEach(() => {
    ...
    global.fetch = jest.fn(async (url, opts) => {
      ...
      if (u.includes('/serve/simple')) {
        return new Response(JSON.stringify({ results: { energy: -2.34, forces: [[0,0,0],[0,0,0]] } }), { status: 200 });
      }
  ...
  test('held index remains at dragged position after mdStep (VR shim)', async () => {
    ...
    const began = api.manipulation.beginDrag(intersector, { planePoint:{x:0,y:0,z:0}, planeNormal:{x:0,y:1,z:0} });
