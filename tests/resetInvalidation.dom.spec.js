...
global.fetch = async (url, opts) => {
  return { ok: true, status: 200, json: async () => ({ results: { energy: -1.23, forces: [[0,0,0]] }, final_energy: -1.23, positions:[[0,0,0]], forces:[[0,0,0]] }) };
};
...
describe('Reset invalidation and no reload', () => {
  test('reset increments userInteractionVersion and cancels stale responses', async () => {
    ...
    delete window.location; window.location = { href: 'http://localhost', assign: jest.fn() };
    const { initNewViewer } = await import('../public/index.js');
    const { buildDesktopPanel } = await import('../public/ui/desktopPanel.js');
    const viewer = await initNewViewer(document.getElementById('viewer'), { elements:[{Z:8}], positions:[{x:0,y:0,z:0}], bonds:[] });
    ...
    await btn.click(); await Promise.resolve();
