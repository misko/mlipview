...
BABYLON.Engine = function(){ this.getRenderingCanvas=()=>({ getBoundingClientRect:()=>({left:0,top:0,width:100,height:100})}); };
BABYLON.Scene = function(){ this.onPointerObservable = { _l:[], add(fn){ this._l.push(fn); }, notify(ev){ this._l.forEach(f=>f(ev)); }, notifyObservers(ev){ this._l.forEach(f=>f(ev)); } }; };
BABYLON.Color4 = function(){}; BABYLON.Vector3 = function(x,y,z){ this.x=x; this.y=y; this.z=z; }; BABYLON.Vector3.prototype.length=function(){return Math.hypot(this.x,this.y,this.z);};
...
BABYLON.MeshBuilder = { CreateSphere:()=>({ thinInstanceSetBuffer(){}, isPickable:true }), CreateCylinder:()=>({ thinInstanceSetBuffer(){}, isPickable:false }) };
...
test('browser-like pointer event selects atom', async () => {
  ...
  const canvas = { addEventListener(type,fn){ if (type==='pointerdown') this._down=fn; if (type==='pointermove') this._move=fn; }, getBoundingClientRect:()=>({left:0,top:0,width:100,height:100}) };
  ...
  return createScene(canvas).then(({ scene }) => {
    ...
    scene.pointerX=10; scene.pointerY=10;
