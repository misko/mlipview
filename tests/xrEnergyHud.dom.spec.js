...
describe('XR HUD energy panel', () => {
  beforeEach(() => {
    ...
    BABYLON.Vector3 =
      ...
      class Vector3 {
        ...
        add(v) {
          return new BABYLON.Vector3(this.x + v.x, this.y + v.y, this.z + v.z);
        }
        subtract(v) {
          return new BABYLON.Vector3(this.x - v.x, this.y - v.y, this.z - v.z);
        }
        scale(s) {
          return new BABYLON.Vector3(this.x * s, this.y * s, this.z * s);
        }
        clone() {
    ...
    BABYLON.GUI.AdvancedDynamicTexture = {
      CreateFullscreenUI: (_name, _opt, _scene) => ({
        _rootContainer: new Node('root'),
        addControl(c) {
          this._rootContainer.addControl(c);
          return c;
        },
        useInvalidateRectOptimization: false,
      }),
    };
    BABYLON.GUI.StackPanel = class StackPanel extends Node {
      constructor(name) {
    ...
    BABYLON.GUI.Rectangle = class Rectangle extends Node {
      constructor(name) {
    ...
    BABYLON.GUI.TextBlock = class TextBlock extends Node {
      constructor(name, text) {
        super(name || 'TextBlock');
        this.text = text || '';
        this.color = '';
        this.fontSize = 0;
    ...
    BABYLON.GUI.Button = class Button extends Node {
      constructor(name) {
        super(name || 'Button');
        this.textBlock = { text: '' };
        this.background = '';
      ...
      static CreateSimpleButton(id, text) {
        const b = new BABYLON.GUI.Button(id);
        b.textBlock.text = text;
        return b;
      }
    };
    // DynamicTexture mock with 2D context
    BABYLON.GUI.Image = class Image extends Node {
    ...
    BABYLON.DynamicTexture = class DynamicTexture {
      constructor() {
        this._ctx = createCanvasCtx();
      }
      getContext() {
        return this._ctx;
      }
    ...
    function createCanvasCtx() {
      ...
      return {
        ...
        font: '',
        measureText(txt) {
          return { width: (txt ? ('' + txt).length : 0) * 6 };
        },
        fillText() {},
      };
    }
    ...
    global.window.viewerApi = {
      ...
      get state() {
        return this._state || (this._state = { dynamics: { energy: -12.345 }, bus });
      },
    };
  ...
  test('adds energy panel to bottom HUD, left of Relax', async () => {
    ...
    const firstName =
      first && (first.name || (first.children && first.children[0] && first.children[0].name));
