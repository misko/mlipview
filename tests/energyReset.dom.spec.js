...
global.fetch = async () => ({ ok:true, status:200, json: async ()=> ({ results:{ energy: -1.0, forces: [[0,0,0]] }, final_energy: -1.0, positions:[[0,0,0]], forces:[[0,0,0]] }) });
...
describe('Energy plot clears on reset', () => {
  test('debugEnergySeriesLength becomes 0 after reset', async () => {
    ...
    `;
    delete window.location; window.location = { href: 'http://localhost' };
