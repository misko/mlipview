...
async function initViewer() {
  ...
  // Lightweight mocks
  jest.mock('../public/render/scene.js', () => ({ createScene: async () => ({ engine:{ stopRenderLoop:()=>{} }, scene:{ render:()=>{}, dispose:()=>{}, onPointerObservable:{ add:()=>{} } }, camera:{} }) }));
  jest.mock('../public/core/pickingService.js', () => ({ createPickingService: () => ({}) }));
  jest.mock('../public/render/moleculeView.js', () => ({ createMoleculeView: () => ({ rebuildBonds:()=>{}, rebuildForces:()=>{} }) }));
...
function mockFetchMDRelax(api){
  ...
  global.fetch = async (url, opts) => {
    ...
    if(u.includes('/serve/relax')){
      return new Response(JSON.stringify({ positions: api.state.positions.map(p=>[p.x+0.01,p.y,p.z]), forces:[], final_energy:-10 }), { status:200, headers:{'Content-Type':'application/json'} });
    }
    if(u.includes('/serve/md')){
      return new Response(JSON.stringify({ positions: api.state.positions.map(p=>[p.x+0.02,p.y,p.z]), forces:[], final_energy:-11 }), { status:200, headers:{'Content-Type':'application/json'} });
    }
    if(u.includes('/serve/simple')){
      return new Response(JSON.stringify({ results:{ energy:-100, forces:[[0,0,0],[0,0,0],[0,0,0]] } }), { status:200, headers:{'Content-Type':'application/json'} });
    }
...
if (typeof Response === 'undefined') {
  global.Response = class { constructor(body, init){ this._body=body; this.status=init.status; this.headers=init.headers||{}; } async json(){ return JSON.parse(this._body); } async text(){ return this._body; } get ok(){ return this.status>=200 && this.status<300; } };
}
...
describe('userInteractionVersion isolation', () => {
  test('mdStep & relaxStep do not bump userInteractionVersion', async ()=>{
    ...
    expect(after.totalInteractionVersion).toBeGreaterThanOrEqual(before.totalInteractionVersion+1);
    cleanup();
  });
});
