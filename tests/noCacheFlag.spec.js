  ...
  beforeEach(() => {
    ...
    global.fetch = jest.fn(async (url, opts) => {
      ...
      if (/\/serve\/relax/.test(String(url))) {
        return { ok: true, json: async () => ({ initial_energy: -1.0, final_energy: -1.1, positions: [[0,0,0]], forces: [[0,0,0]], steps_completed: 1, cache_key: 'ckey-234' }) };
      }
      if (/\/serve\/md/.test(String(url))) {
        return { ok: true, json: async () => ({ initial_energy: -1.1, final_energy: -1.2, positions: [[0,0,0]], velocities: [[0,0,0]], forces: [[0,0,0]], steps_completed: 1, temperature: 300, cache_key: 'ckey-345' }) };
      }
