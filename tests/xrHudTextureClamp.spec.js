/**
 * xrHudTextureClamp.spec
 * Ensures large scaling triggers GUI texture dimension clamping to configured maxHUDTextureDim
 * and records metadata about raw vs final sizes.
 */
...
if(!B.MeshBuilder) B.MeshBuilder={}; if(!B.MeshBuilder.CreatePlane) B.MeshBuilder.CreatePlane=(n,o,s)=>({ name:n, opts:o, scene:s, position:v(), rotation:{y:0}, isPickable:true, metadata:{} });
...
function makeScene(){ return { getEngine:()=>({ getCaps:()=>({ maxTextureSize:1024 }) }), activeCamera:{ fov:Math.PI/2, position:v(), getDirection(ax){ if(ax===B.Axis.Z) return v(0,0,1); if(ax===B.Axis.Y) return v(0,1,0); return v(); } }, onBeforeRenderObservable:{ _l:[], add(fn){ this._l.push(fn); return fn; } } }; }

describe('XR HUD texture clamping', () => {
  test('top energy texture clamped when exceeding max', () => {
    if(window.__XR_HUD_FALLBACK){ delete window.__XR_HUD_FALLBACK; delete window.__XR_HUD_FALLBACK_TOP; }
    const scene = makeScene();
    // Use very large scale to force raw texture > maxTextureSize (1024)
    const res = ensureWorldHUD({ scene, topEnergyScaleMult:8, maxHUDTextureDim:1024 });
    expect(res).toBeTruthy();
    // Metadata present on top plane
    const md = res.planeTop.metadata || {};
    expect(md.hudTexSize).toBeTruthy();
    const { rawW, rawH, w, h, clamped, max } = md.hudTexSize;
    expect(rawW).toBeGreaterThan(1024);
    expect(rawH).toBeGreaterThan(1024);
