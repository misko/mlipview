...
describe('XR HUD top energy scale multiplier', () => {
  test('scale multiplier doubles width/height roughly', () => {
    ...
    const r1 = ensureWorldHUD({ scene: s1, topEnergyScaleMult: 1 });
    s1.onBeforeRenderObservable._l.forEach((fn) => fn());
    if (window.__XR_HUD_FALLBACK) {
      delete window.__XR_HUD_FALLBACK;
      delete window.__XR_HUD_FALLBACK_TOP;
    }
    const s2 = makeScene();
    const r2 = ensureWorldHUD({ scene: s2, topEnergyScaleMult: 2 });
    s2.onBeforeRenderObservable._l.forEach((fn) => fn());
