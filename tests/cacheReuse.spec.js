...
describe('cache reuse flow chooses *_from_cache endpoints', () => {
  ...
  beforeAll(() => {
    ...
    global.document = global.document || {
      getElementById: (id)=> ({ id, textContent:'', width: 300, height: 100, getContext: ()=> ({ clearRect(){}, beginPath(){}, moveTo(){}, lineTo(){}, stroke(){}, arc(){}, fill(){}, fillText(){} }) }),
      createElement: (tag)=> ({ tagName: tag.toUpperCase(), getContext: ()=> ({}) }),
    };
    // Minimal canvas for scene.js
    const canvas = { getContext: ()=>({}), addEventListener: ()=>{}, getBoundingClientRect: ()=>({left:0,top:0,width:800,height:600}) };
    // Attach energy canvas element lookup
    document.getElementById = (id)=>{
      if(id==='energyCanvas') return { width: 320, height: 80, getContext: ()=> ({ clearRect(){}, beginPath(){}, moveTo(){}, lineTo(){}, stroke(){}, arc(){}, fill(){}, fillText(){} }) };
    ...
    global.fetch = jest.fn(async (url, init) => {
      ...
      if(/\/serve\/simple$/.test(url)){
        return makeJsonResponse({ results: { energy: -1.23, forces: [[0,0,0],[0,0,0]] }, cache_key: 'K1' });
      }
      if(/\/serve\/relax_from_cache$/.test(url)){
        // Return new cache key each call to ensure client updates it
        return makeJsonResponse({ initial_energy: -1.23, final_energy: -1.5, positions: [[0,0,0],[0,0,0]], forces: [[0,0,0],[0,0,0]], steps_completed: 1, calculator: 'uma', cache_key: 'K2' });
      }
      if(/\/serve\/relax$/.test(url)){
        return makeJsonResponse({ initial_energy: -1.23, final_energy: -1.4, positions: [[0,0,0],[0,0,0]], forces: [[0,0,0],[0,0,0]], steps_completed: 1, calculator: 'uma', cache_key: 'K2' });
      }
      if(/\/serve\/simple_from_cache$/.test(url)){
        return makeJsonResponse({ results: { energy: -1.6, forces: [[0,0,0],[0,0,0]] }, cache_key: 'K3' });
      }
      if(/\/serve\/md_from_cache$/.test(url)){
        return makeJsonResponse({ initial_energy: -1.6, final_energy: -1.55, positions: [[0,0,0],[0,0,0]], velocities: [[0,0,0],[0,0,0]], forces: [[0,0,0],[0,0,0]], steps_completed: 1, temperature: 300, calculator: 'uma', cache_key: 'K4' });
      }
      // Default OK empty
  ...
  test('uses from_cache for relax and simple when no user interaction between calls', async () => {
    // Initialize viewer
    const canvas = { getContext: ()=>({}), addEventListener: ()=>{}, getBoundingClientRect: ()=>({left:0,top:0,width:800,height:600}) };
    const api = await initNewViewer(canvas, { elements: ['O','H'], positions: [[0,0,0],[1,0,0]], bonds: [] });
