...
const MeshBuilder = {
  CreateSphere: (n) => new Mesh(n),
  CreateCylinder: (n) => new Mesh(n),
  CreateLines: (n) => new Mesh(n),
};
class TransformNode {
  constructor(name) {
    this.name = name;
...
class Scene {
  ...
  createPickingRay() {
    return { origin: new Vector3(0, 0, 0), direction: new Vector3(0, 1, 0) };
  }
  pick() {
    return { hit: false };
  }
...
global.fetch = jest.fn(async (url, opts) => {
  ...
  const body =
    fetchCallCount === 1
      ...
      : { results: { energy: -12.5, forces: secondForces } };
...
describe('force update after perturbation', () => {
  test('forces change after atom displacement triggers recompute', async () => {
    ...
    api.state.positions[0].x += 5.0;
    api.state.positions[0].y += 4.0;
    api.state.positions[0].z += -3.0;
    ...
    for (let t = 0; t < 40; t++) {
      fgAfter = api.view._internals.forceGroups.get('force');
      afterMatrix = fgAfter?.master?._buffers['matrix'];
