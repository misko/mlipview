...
// obsolete inline haveServer (now using helpers/server.js) but keep minimal version hitting /serve/health for backward safety

async function callRelax(calculator, steps) {
  ...
  const body = { atomic_numbers, coordinates, steps, calculator };
...
describe('continuous relax run parity', () => {
  test('final energy within tolerance of aggregate relax', async () => {
    ...
    if (!up) {
      console.warn('[relax run parity] server not reachable; skipping');
      return;
    }
