...
global.fetch = async (url, opts={}) => {
  if(/\/serve\/simple$/.test(url)){
    const body = JSON.stringify({ results:{ energy:-1.0, forces:[[0,0,0]], stress:[0,0,0,0,0,0] } });
    return { ok:true, status:200, json: async ()=> JSON.parse(body), text: async ()=> body };
  }
  if(/\/serve\/md$/.test(url)){
    // Echo minimal MD JSON; after a few steps we let loop finish naturally.
    const body = JSON.stringify({ positions:[[0,0,0]], velocities:[[0,0,0]], forces:[[0,0,0]], final_energy:-1.0, steps_completed:1, temperature:298 });
    return { ok:true, status:200, json: async ()=> JSON.parse(body), text: async ()=> body };
  }
  if(/\/serve\/relax$/.test(url)){
    const body = JSON.stringify({ initial_energy:-1, final_energy:-1, positions:[[0,0,0]], forces:[[0,0,0]], steps_completed:1 });
    return { ok:true, status:200, json: async ()=> JSON.parse(body), text: async ()=> body };
  }
...
test('auto-start MD run resets button text', async () => {
  ...
  await mod.initNewViewer(document.getElementById('viewer'), { elements:[{Z:8}], positions:[{x:0,y:0,z:0}], bonds:[] });
