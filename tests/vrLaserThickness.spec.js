...
function makeScene(pickSequence) {
  ...
  const scene = {
    ...
    pickWithRay: (ray) => {
      if (frame < pickSequence.length) {
        const res = pickSequence[frame];
        frame++;
        return res;
      }
      frame++;
    ...
    getMeshByName(name) {
      return name === 'molecule_root' ? fakeMaster : null;
    },
    step(n = 1) {
      for (let i = 0; i < n; i++) {
...
function fabricateController(vr) {
  ...
  const fake = {
    uniqueId: 'shimControllerLaser',
    pointer: {
      getAbsolutePosition: () => new BABYLON.Vector3(0, 1, 0),
...
const view = {
  resolveAtomPick: (p) => (p === atomPick ? { kind: 'atom', index: 1 } : null),
