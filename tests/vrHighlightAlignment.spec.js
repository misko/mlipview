...
class Quaternion {
  ...
  static RotationAxis(axis, angle){
    const half = angle/2; const s = Math.sin(half); const c = Math.cos(half);
    const ax = axis.x, ay = axis.y, az = axis.z;
    return new Quaternion(ax*s, ay*s, az*s, c);
  }
}
class StandardMaterial { constructor(name,scene){ this.name=name; this.diffuseColor={}; this.emissiveColor={}; this.alpha=1; } }
class Matrix { constructor(){ this.m=new Float32Array(16);} static Compose(){ return new Matrix(); } }
function makeMaster(name){ return { name, thinInstanceSetBuffer(){}, position:Vector3.Zero(), scaling:new Vector3(1,1,1), rotationQuaternion:Quaternion.Identity(), isPickable:true }; }
const MeshBuilder = { CreateSphere:(name)=>makeMaster(name), CreateCylinder:(name)=>makeMaster(name), CreateLines:()=>({}) };

Object.assign(BABYLON, { Vector3, Quaternion, StandardMaterial, Matrix, MeshBuilder, Axis:{ X:new Vector3(1,0,0), Y:new Vector3(0,1,0), Z:new Vector3(0,0,1) } });

function makeScene(){ return { meshes:[], onBeforeRenderObservable:{ add(){} }, getMeshByName(){ return null; } }; }
