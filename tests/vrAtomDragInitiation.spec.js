...
class Vector3 { constructor(x=0,y=0,z=0){ this.x=x; this.y=y; this.z=z; } clone(){ return new Vector3(this.x,this.y,this.z); } subtract(v){ return new Vector3(this.x-v.x,this.y-v.y,this.z-v.z); } length(){ return Math.hypot(this.x,this.y,this.z); } normalize(){ const L=this.length()||1; this.x/=L; this.y/=L; this.z/=L; return this; } add(v){ return new Vector3(this.x+v.x,this.y+v.y,this.z+v.z); } scale(f){ return new Vector3(this.x*f,this.y*f,this.z*f); } addInPlace(v){ this.x+=v.x; this.y+=v.y; this.z+=v.z; return this; } }
class Quaternion { constructor(x=0,y=0,z=0,w=1){ this.x=x; this.y=y; this.z=z; this.w=w; } static Identity(){ return new Quaternion(); } static RotationAxis(){ return new Quaternion(); } multiply(q){ return new Quaternion(this.x+q.x,this.y+q.y,this.z+q.z,this.w*q.w); } }
const Axis = { X:new Vector3(1,0,0), Y:new Vector3(0,1,0), Z:new Vector3(0,0,1) };
class Ray { constructor(o,d,l=100){ this.origin=o; this.direction=d; this.length=l; } }
Object.assign(global.BABYLON,{ Vector3, Quaternion, Axis, Ray, MeshBuilder:{}, Matrix: class{}, Color3: class{}, StandardMaterial: class{} });
...
describe('vr atom drag initiation', () => {
  ...
  test('hold after atom selection begins drag', () => {
    ...
    const startedPre = manipulation.beginDrag(()=>({ x: mol.positions[1].x, y: mol.positions[1].y, z: mol.positions[1].z }));
    ...
    let step=0;
    function intersector(){ return { x: mol.positions[1].x + 0.1*step, y: mol.positions[1].y, z: baseZ + 0.2*step }; }
