...
describe('continuous MD run stability', () => {
  test('200 MD steps produce finite positions and energy', async () => {
    if (!(await haveServer())) {
      console.warn('[md run stability] server not reachable; skipping');
      return;
    }
    const api = await setupViewer();
    const res = await api.startMDContinuous({
      steps: 200,
      calculator: 'uma',
      temperature: 298,
      timestep_fs: 1.0,
    ...
    if (trace.length <= 2) {
      ...
      trace = [];
      for (let i = 0; i < 5; i++) {
        await api.mdStep({ calculator: 'uma', temperature: 298 });
        trace.push(api.state.dynamics.energy);
      }
    }
