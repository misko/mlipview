...
if (!B.MeshBuilder) B.MeshBuilder = { CreateSphere:(n,o,s)=>({ name:n, thinInstanceSetBuffer(){}, getChildren(){return[];}, }), CreateCylinder:(n,o,s)=>({ name:n, thinInstanceSetBuffer(){}, getChildren(){return[];}, }) };
if (!B.Matrix) B.Matrix = class { constructor(){ this.m=new Float32Array(16);} static Compose(){ return new B.Matrix(); } };
if (!B.Vector3) B.Vector3 = class { constructor(x=0,y=0,z=0){ this.x=x; this.y=y; this.z=z;} length(){return Math.hypot(this.x,this.y,this.z);} add(v){ return new B.Vector3(this.x+v.x,this.y+v.y,this.z+v.z);} subtract(v){ return new B.Vector3(this.x-v.x,this.y-v.y,this.z-v.z);} scale(f){ return new B.Vector3(this.x*f,this.y*f,this.z*f);} static Dot(a,b){ return a.x*b.x+a.y*b.y+a.z*b.z;} static Cross(a,b){ return new B.Vector3(a.y*b.z-a.z*b.y,a.z*b.x-a.x*b.z,a.x*b.y-a.y*b.x);} normalizeToNew(){ const L=this.length()||1; return new B.Vector3(this.x/L,this.y/L,this.z/L);} };
if (!B.Quaternion) B.Quaternion = class { constructor(x=0,y=0,z=0,w=1){ this.x=x; this.y=y; this.z=z; this.w=w; } static Identity(){ return new B.Quaternion(0,0,0,1); } static RotationAxis(axis,angle){ const h=angle/2; const s=Math.sin(h); return new B.Quaternion(axis.x*s,axis.y*s,axis.z*s,Math.cos(h)); } };
if (!B.Axis) B.Axis = { X:new B.Vector3(1,0,0), Y:new B.Vector3(0,1,0), Z:new B.Vector3(0,0,1) };

function makeSceneStub(){
  // Provide onBeforeRenderObservable so frame update path runs.
  return { onPointerObservable:{ add(){} }, onBeforeRenderObservable:{ _c:[], add(fn){ this._c.push(fn); }, run(){ for(const f of this._c) f(); } }, meshes:[] };
}

// Helper to manually invoke frame observers (simulate render loop)
function step(scene, n=1){ for(let i=0;i<n;i++) scene.onBeforeRenderObservable.run(); }
...
describe('bond highlight rotation inheritance', () => {
  test('bond highlight midpoint rotates with master', () => {
    ...
    const initialPos = { x:highlight.bond.position.x, y:highlight.bond.position.y, z:highlight.bond.position.z };
