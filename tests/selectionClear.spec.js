...
function makeView(state){
  return {
    resolveAtomPick(pick){ return pick && pick.hit ? { kind:'atom', index:0 } : null; },
    resolveBondPick(){ return null; }
  };
}

describe('selection clear via empty click', () => {
  test('select then empty click clears', () => {
    const state = createMoleculeState({ elements:['C'], positions:[{x:0,y:0,z:0}], bonds:[] });
    const selection = createSelectionService(state);
    const scene = { pointerX:0, pointerY:0, pick(){ return { hit:true }; }, onPointerObservable:{ add(){} } };
