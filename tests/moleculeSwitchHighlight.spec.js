...
describe('molecule switch clears highlight and hides empty masters', () => {
  test('switch from two-atom to single-atom', async () => {
    const molState = { elements:['C','C'], positions:[{x:0,y:0,z:0},{x:1.4,y:0,z:0}], bonds:[{i:0,j:1,opacity:1}], selection:{ kind:'bond', data:{ i:0,j:1 } }, showCell:false, showGhostCells:false, cell:{enabled:false}, bus: createBus() };
    ...
    for (const [,g] of view._internals.bondGroups) {
      if (g.mats.length === 0) {
        expect(g.master.isVisible === false || (g.master.isEnabled && !g.master.isEnabled())).toBe(true);
      }
    }
  });
});
