        ...
        if (msg && msg.simulationParams) {
          ...
          setTimeout(() => {
            ws.injectTestResult({
              ...
              forces: mdForces(mdStepCount),
              energy: -3.0 - 0.05 * mdStepCount,
            });
          }, 0);
        } else if (msg && msg.type) {
          // Idle compute baseline after init
          setTimeout(() => {
            ws.injectTestResult({ energy: -3.0, forces: mdForces(0) });
          }, 0);
        }
      } catch {}
    });
