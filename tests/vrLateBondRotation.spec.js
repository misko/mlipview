...
class Vector3 { constructor(x=0,y=0,z=0){ this.x=x; this.y=y; this.z=z; }
  ...
  normalizeToNew(){ const L=this.length()||1; return new Vector3(this.x/L,this.y/L,this.z/L); }
  ...
  applyRotationQuaternionInPlace(q){ // simplified quaternion rotate
    const x=this.x,y=this.y,z=this.z; const qx=q.x,qy=q.y,qz=q.z,qw=q.w;
...
class Quaternion { constructor(x=0,y=0,z=0,w=1){ this.x=x; this.y=y; this.z=z; this.w=w; } static Identity(){ return new Quaternion(); } clone(){ return new Quaternion(this.x,this.y,this.z,this.w); } copyFrom(q){ this.x=q.x; this.y=q.y; this.z=q.z; this.w=q.w; }
  static RotationAxis(axis, angle){ const s=Math.sin(angle/2); return new Quaternion(axis.x*s,axis.y*s,axis.z*s,Math.cos(angle/2)); }
}
...
const MeshBuilder = { CreateSphere:(n, _opts, scene)=>{ const m={ name:n, position:new Vector3(), scaling:new Vector3(1,1,1), isPickable:true, thinInstanceEnablePicking:true, thinInstanceSetBuffer(){ this._thinInstanceMatricesData=arguments[1]; }, material:null }; scene?.meshes?.push(m); return m; }, CreateCylinder:(n,_opts,scene)=>{ const m={ name:n, position:new Vector3(), scaling:new Vector3(1,1,1), isPickable:true, thinInstanceEnablePicking:true, thinInstanceSetBuffer(){ this._thinInstanceMatricesData=arguments[1]; }, material:null }; scene?.meshes?.push(m); return m; } };
class TransformNode { constructor(name){ this.name=name; this.position=new Vector3(); this.scaling=new Vector3(1,1,1); this._children=[]; } getClassName(){ return 'TransformNode'; } addChild(m){ m.parent=this; this._children.push(m); } }
class Matrix { constructor(){ this.m=new Float32Array(16); } static Compose(scale, rot, trans){ const mat=new Matrix(); // ultra-simplified compose (no real rotation math)
  // Set scale on diagonal
  mat.m[0]=scale.x; mat.m[5]=scale.y; mat.m[10]=scale.z; mat.m[15]=1;
  // Translation
  mat.m[12]=trans.x; mat.m[13]=trans.y; mat.m[14]=trans.z;
  return mat; }
}
Vector3.Up = ()=> new Vector3(0,1,0);
Object.assign(BABYLON,{ Vector3, Quaternion, StandardMaterial, Color3, MeshBuilder, TransformNode, Matrix, Axis:{X:new Vector3(1,0,0),Y:new Vector3(0,1,0),Z:new Vector3(0,0,1)} });

function makeScene(){
  return {
    meshes:[],
    transformNodes:[],
    onBeforeRenderObservable:{ _cbs:[], add(fn){ this._cbs.push(fn); } },
    getTransformNodeByName(name){ return this.transformNodes.find(t=>t.name===name); },
    // Simulate a render frame
    step(){ for(const cb of this.onBeforeRenderObservable._cbs) cb(); }
  };
}
...
describe('vr late bond rotation', () => {
  test('late-created bond follows subsequent rotations', () => {
    ...
    const mol = createMoleculeState({
      elements:['O','C','H'],
      positions:[ {x:0,y:0,z:0}, {x:2,y:0,z:0}, {x:0,y:2,z:0} ],
    ...
    root.rotationQuaternion = new BABYLON.Quaternion(0, Math.sin(Math.PI/4), 0, Math.cos(Math.PI/4));
    scene.step();
  ...
  expect(bondMaster.rotationQuaternion == null).toBe(true);
    ...
    root.rotationQuaternion = new BABYLON.Quaternion(0, Math.sin(Math.PI/2), 0, Math.cos(Math.PI/2));
    scene.step();
    ...
    expect(Math.abs(root.rotationQuaternion.y)).toBeGreaterThan(0.70);
    // 2. Bond master itself should not carry its own rotationQuaternion (inherit root) after creation.
    expect(bondMaster.rotationQuaternion == null || bondMaster.rotationQuaternion === undefined).toBe(true);
    // 3. Local midpoint unchanged (no per-master adjustment), which was prior cause of apparent freeze until root centralization.
    const buffer2 = bondMaster._thinInstanceBufferMatrices || bondMaster._thinInstanceMatrixBuffer || bondMaster._thinInstanceMatricesData;
