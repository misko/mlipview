...
describe('camera suppression during atom drag', () => {
  test('detach/attach called around drag', () => {
    ...
    const selection = { clickAtom:(i)=>{ st.selection={kind:'atom',data:{index:i}}; st.markSelectionChanged(); }, clickBond:()=>{}, clear:()=>{} , get:()=>st.selection };
    const manip = createManipulationService(st);
  const detachSpy = function(){ detachSpy.calls++; };
  detachSpy.calls=0;
  const attachSpy = function(){ attachSpy.calls++; };
  attachSpy.calls=0;
