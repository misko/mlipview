...
function patchFetchOnce({ match, delayMs = 50, respond }) {
  ...
  global.fetch = async (url, opts) => {
    if (!used && typeof url === 'string' && url.includes(match)) {
      ...
      return new Response(JSON.stringify(respData), { status: 200, headers: { 'Content-Type': 'application/json' } });
    }
  ...
  test('older md response discarded due to newer simulation application', async () => {
    ...
    global.fetch = async (url, opts) => {
      if (typeof url === 'string' && url.includes('/md')) {
        mdCallCount++;
        if (mdCallCount === 1) { // slow first
          await delay(120);
          return new Response(JSON.stringify({ positions: api.state.positions.map(p=>[p.x+1,p.y,p.z]), forces:[], final_energy: info0.energy??-2 }), { status:200, headers:{'Content-Type':'application/json'} });
        } else { // fast second accepted
          return new Response(JSON.stringify({ positions: api.state.positions.map(p=>[p.x+0.01,p.y,p.z]), forces:[], final_energy: -3 }), { status:200, headers:{'Content-Type':'application/json'} });
        }
      }
  ...
  test('relax step accepted when no user interaction', async () => {
    ...
    global.fetch = async (url, opts) => {
      if (typeof url === 'string' && url.includes('/relax')) {
        return new Response(JSON.stringify({ positions: api.state.positions.map(p=>[p.x+0.02,p.y,p.z]), forces:[], final_energy: -5 }), { status:200, headers:{'Content-Type':'application/json'} });
      }
      return origFetch(url, opts);
    };
