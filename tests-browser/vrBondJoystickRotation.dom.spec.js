...
function makeScene() {
  ...
  return {
    ...
    meshes: [],
    transformNodes: [],
    onBeforeRenderObservable: { _cbs: [], add(fn){ this._cbs.push(fn); return fn; }, remove(fn){ const i=this._cbs.indexOf(fn); if(i>=0) this._cbs.splice(i,1); } },
    pickWithRay(){ return { hit:false }; },
    getEngine(){ return { getRenderingCanvas(){ return { getBoundingClientRect(){ return { left:0, top:0 }; } }; } }; },
    activeCamera: { getDirection(){ return new BABYLON.Vector3(1,0,0); }, position: new BABYLON.Vector3() }
  };
}

function step(scene, n=1){ for(let i=0;i<n;i++){ try { for(const fn of scene.onBeforeRenderObservable._cbs) fn(); } catch {} } }
...
function makeController({ id='c1' }={}){
  ...
  return {
    ...
    motionController: { getComponent(){ return { pressed:false }; } },
    // Minimal transform methods for ray/laser code; not used in this test
    getDirection(){ return new BABYLON.Vector3(0,0,1); },
    pointer: { getAbsolutePosition(){ return new BABYLON.Vector3(); }, getDirection(){ return new BABYLON.Vector3(0,0,1); } }
  };
}
...
describe('VR bond joystick rotation', () => {
  test('stick X produces continuous bond rotation over frames', async () => {
    ...
    const st = createMoleculeState({
      ...
      positions: [ {x:0,y:0,z:0}, {x:1.5,y:0,z:0}, {x:2.2,y:0.4,z:0} ],
  ...
  const xr = { input: { controllers: [], onControllerAddedObservable: { add(){} }, onControllerRemovedObservable: { add(){} } }, baseExperience: { sessionManager: { onXRSessionInit: { add(){} }, onXRSessionEnded: { add(){} } }, camera: { position: new BABYLON.Vector3(), getDirection(){ return new BABYLON.Vector3(0,0,1); } } } };
