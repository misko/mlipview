...
function makeViewer(){
  ...
  const state = {
    bus,
    elements: ['H','H','O'], positions: [{x:0,y:0,z:0},{x:1,y:0,z:0},{x:0,y:1,z:0}], bonds: [],
...
describe('MD initial requests use XYZ temperature when present', ()=>{
  beforeEach(()=>{ document.body.innerHTML = '<div id="hud"></div>'; delete window.__MLIP_TARGET_TEMPERATURE; window.__MLIP_DEBUG_MD_TEMP = true; });
  ...
  test('first 10 MD calls use 500K', async ()=>{
    ...
    global.fetch = async (url, opts)=>{
      ...
      const response = { positions: viewer.state.positions.map(p=>[p.x,p.y,p.z]), forces: viewer.state.positions.map(()=>[0,0,0]), final_energy: 0, velocities: viewer.state.positions.map(()=>[0,0,0]), temperature: 500 };
      return {
        ok: true,
        status: 200,
        json: async ()=> response,
    ...
    try {
      ...
      const api = await mod.initNewViewer(canvas, { elements: viewer.state.elements, positions: viewer.state.positions, bonds: [] });
