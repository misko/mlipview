...
beforeAll(() => {
  ...
  if (!BABYLON.Color3)
    BABYLON.Color3 = function (r = 1, g = 1, b = 1) {
      this.r = r;
      this.g = g;
      this.b = b;
      this.clone = () => new BABYLON.Color3(r, g, b);
      this.scale = (s) => new BABYLON.Color3(r * s, g * s, b * s);
    };
...
describe('acetic acid: ghosts render and API includes cell', () => {
  test('ghost instances appear and payload carries cell/pbc', async () => {
      ...
      try {
        ...
        return {
          ...
          json: async () => ({
            results: {
              energy: 0,
              forces: Array((body.atomic_numbers || []).length).fill([0, 0, 0]),
              stress: [0, 0, 0, 0, 0, 0],
            },
          }),
          text: async () => 'ok',
    ...
    const atomic_numbers = viewer.state.elements.map((e) =>
      e === 'H' ? 1 : e === 'C' ? 6 : e === 'O' ? 8 : 0
    );
    ...
    if (viewer.state.showCell && viewer.state.cell && viewer.state.cell.enabled) {
      body.cell = [
        [viewer.state.cell.a.x, viewer.state.cell.a.y, viewer.state.cell.a.z],
        [viewer.state.cell.b.x, viewer.state.cell.b.y, viewer.state.cell.b.z],
        [viewer.state.cell.c.x, viewer.state.cell.c.y, viewer.state.cell.c.z],
      ];
      body.pbc = [true, true, true];
    }
    await fetch(base + getEndpointSync('simple'), {
      method: 'POST',
