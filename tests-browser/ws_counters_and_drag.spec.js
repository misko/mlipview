...
describe('WS userInteraction counter gating and drag exclusion', () => {
  ...
  test('dragging atom prevents overwrite by incoming frame', async () => {
    ...
    api.manipulation.beginDrag(() => ({ x: startPos.x, y: startPos.y, z: startPos.z }), {
      source: 'desktop',
      planePoint: startPos,
    ...
    const newPositions = api.state.positions.map((p, idx) =>
      idx === 0 ? [p.x + 5, p.y + 5, p.z + 5] : [p.x, p.y, p.z]
    );
    if (typeof window.__ON_WS_RESULT__ === 'function') {
      window.__ON_WS_RESULT__({
        positions: newPositions,
        forces: [],
        energy: 0,
        userInteractionCount: curUIC,
        simStep: 1,
      });
    }
