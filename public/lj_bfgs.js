...
export async function bfgsOptimize({
  positions,
  fmax = 0.05,
  maxSteps = 1000,
  maxStep = 0.2,
  ...
  function unflatten(vec) {
    for (let i = 0; i < n; i++) {
      positions[i][0] = vec[3 * i];
      positions[i][1] = vec[3 * i + 1];
      positions[i][2] = vec[3 * i + 2];
    }
  }
  ...
  for (let k = 0; k < maxSteps; k++) {
    ...
    for (let i = 0; i < dim; i++) {
      let s = 0;
      const row = H[i];
      for (let j = 0; j < dim; j++) s += row[j] * g[j];
      p[i] = -s;
    }
    ...
    if (maxComp > maxStep) {
      const scale = maxStep / maxComp;
      for (let i = 0; i < dim; i++) p[i] *= scale;
    }
    // Line search (Armijo)
    const c1 = 1e-4;
    let alpha = 1.0;
    ...
    for (let i = 0; i < dim; i++) s[i] = x[i] - x_old[i];
    const y = new Float64Array(dim);
    for (let i = 0; i < dim; i++) y[i] = newG[i] - g[i];
    const ys = dot(y, s);
    if (ys > 1e-12) {
      const Hy = matVec(H, y);
      const yHy = dot(y, Hy);
...
export async function optimizeWaterExample() {
  ...
  const res = await bfgsOptimize({
    positions,
    fmax: 0.05,
    maxSteps: 500,
    maxStep: 0.2,
    compute: async (p) => ljEnergyForces(p),
  });
...
export function createBFGSStepper({ positions, fmax = 0.05, maxStep = 0.2, compute }) {
  ...
  async function ensureEval() {
    if (!evalCache) {
      ...
      evalCache = { energy, forces, g: flattenForces(forces), fmax: maxForceMag(forces) };
      history.push({ step: history.length, energy, fmax: evalCache.fmax });
    }
    return evalCache;
  }
  function flatten(posArr) {
  ...
  return {
    ...
    async step() {
      ...
      if (Math.max(...F.map((f) => Math.hypot(...f))) < fmax)
        return { converged: true, energy: E, fmax: history[history.length - 1].fmax };
      // p = -H g
      const dimLocal = g.length;
      const p = new Float64Array(dimLocal);
      for (let i = 0; i < dimLocal; i++) {
        const row = H[i];
        let s = 0;
        for (let j = 0; j < dimLocal; j++) s += row[j] * g[j];
        p[i] = -s;
      }
      // limit step
      let maxComp = 0;
      ...
      while (true) {
        ...
        if (energy <= E + c1 * alpha * gdotp || alpha < 1e-6) {
          newEval = { energy, forces, g: newG };
          break;
        }
        alpha *= 0.5;
      }
      ...
      for (let i = 0; i < s.length; i++) s[i] = alpha * p[i];
      const y = new Float64Array(p.length);
      for (let i = 0; i < y.length; i++) y[i] = newEval.g[i] - g[i];
      const ys = dot(y, s);
      if (ys > 1e-12) {
        const rho = 1 / ys;
        // (I - rho s y^T) H (I - rho y s^T) + rho s s^T
        const dimL = p.length;
        const temp = Array.from({ length: dimL }, () => new Float64Array(dimL));
        for (let i = 0; i < dimL; i++) {
          for (let j = 0; j < dimL; j++) {
            let Hij = H[i][j];
            let corr = 0;
            for (let q = 0; q < dimL; q++) corr += H[i][q] * (-rho * y[q] * s[j]);
            temp[i][j] = Hij + corr;
          }
        }
        ...
        for (let i = 0; i < dimL; i++) {
          for (let j = 0; j < dimL; j++) {
            let val = temp[i][j];
            let corr = 0;
            for (let q = 0; q < dimL; q++) corr += -rho * s[i] * y[q] * temp[q][j];
            newH[i][j] = val + corr + rho * s[i] * s[j];
          }
        }
        H = newH;
    ...
    getCurrent: async () => {
      const e = await ensureEval();
      return { energy: e.energy, forces: e.forces, fmax: e.fmax };
    },
  };
}
