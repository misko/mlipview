...
export function createLocalLJProvider(molState, opts = {}) {
  ...
  async function compute({ wantStress = true, positions } = {}) {
    ...
    if (wantStress && molState.dynamics.stress) {
      ...
      stress = {
        tensor: { xx: s.xx, yy: s.yy, zz: s.zz, xy: s.xy, xz: s.xz, yz: s.yz },
        voigt: [s.xx, s.yy, s.zz, s.yz, s.xz, s.xy],
      };
    }
    return {
      energy: undefined, // local model presently accumulates energy but not exposed externally yet
      forces,
      stress,
    };
  }
