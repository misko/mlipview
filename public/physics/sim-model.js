...
export function createSimulationState({ Z, positions, cell, masses } = {}) {
  ...
  if (masses) {
    for (let i = 0; i < n; i++) mass[i] = masses[i];
  } else {
    for (let i = 0; i < n; i++) mass[i] = defaultMassForZ(Z[i]);
  }
...
export function stressNorm(s) {
  return Math.sqrt(
    s.xx * s.xx + s.yy * s.yy + s.zz * s.zz + 2 * (s.xy * s.xy + s.xz * s.xz + s.yz * s.yz)
  );
}
...
export function computeKineticAndTemp(state) {
  ...
  for (let i = 0; i < mass.length; i++) {
    ...
    const vx = vel[3 * i],
      vy = vel[3 * i + 1],
      vz = vel[3 * i + 2]; // mass in amu -> convert to kg? We keep internal K relative scale.
...
function angleDeg(a, b) {
  const d = dot(a, b) / (vecLen(a) * vecLen(b) || 1);
...
export function stressVoigt(stress) {
  return [stress.xx, stress.yy, stress.zz, stress.yz, stress.xz, stress.xy];
}
export function setStressFromVoigt(stress, arr) {
  [stress.xx, stress.yy, stress.zz, stress.yz, stress.xz, stress.xy] = arr;
}

export function updatePositions(state, newPos) {
  for (let i = 0; i < state.Z.length; i++) {
    state.pos[3 * i] = newPos[3 * i];
    state.pos[3 * i + 1] = newPos[3 * i + 1];
    state.pos[3 * i + 2] = newPos[3 * i + 2];
  }
}
