...
function quatNormalize(q){
  const len=Math.hypot(q.w,q.x,q.y,q.z)||1; return { x:q.x/len,y:q.y/len,z:q.z/len,w:q.w/len };
}
function quatFromAxisAngle(axis, angle){
  const h=angle*0.5; const s=Math.sin(h); return { x:axis[0]*s, y:axis[1]*s, z:axis[2]*s, w:Math.cos(h) };
}
function rotateVec(q,v){ // v:[x,y,z]
...
export function applyIncrement(accQ, dYaw, dPitch, cameraRight, opts={}){
  ...
  const rAxis = [ cameraRight[0]/rightNormLen, cameraRight[1]/rightNormLen, cameraRight[2]/rightNormLen ];
