...
function getState(viewer){
  const running = (()=>{ try { return viewer?.getMetrics?.().running || null; } catch { return null; } })();
...
export function buildXRControlsModel(opts){
  ...
  const reload = opts?.reloadPage || (()=>{ try { if (typeof location !== 'undefined' && typeof location.reload === 'function') location.reload(); } catch {} });
  const notify = (s)=>{ try { opts?.onStateChange && opts.onStateChange(s); } catch {} };

  function setSimulation(mode){
    const v = getViewer(); if(!v) return false;
    try {
      const current = (()=>{ try { return v?.getMetrics?.().running || null; } catch { return null; } })();
      // Toggle semantics: pressing active relax/md switches to off; pressing off while off stays off
      if (mode === 'relax') {
        if (current === 'relax') { v.stopSimulation?.(); }
        else { v.stopSimulation?.(); v.startRelaxContinuous?.({}); }
      } else if (mode === 'md') {
        if (current === 'md') { v.stopSimulation?.(); }
        else { v.stopSimulation?.(); v.startMDContinuous?.({}); }
      } else { // off
        v.stopSimulation?.();
      }
      notify(getState(v));
  ...
  function reset(){
    const v = getViewer();
      ...
      try { console.log('[XR][HUD] Reset pressed', { hasViewer: !!v, t: new Date().toISOString() }); } catch {}
      if(!v) return false;
      try {
        if (v && typeof v.resetToInitialPositions === 'function') {
          ...
          Promise.resolve(res).then((ok)=>{
            ...
            try { console.log('[XR][Reset] completed', { ok: !!ok, ms: Math.round((t1 - t0)*100)/100 }); } catch {}
          }).catch(e=>{ try { console.warn('[XR][Reset] failed', e?.message||e); } catch{} });
          notify(getState(v));
          return true;
        }
        try { console.warn('[XR][Reset] viewer missing resetToInitialPositions(), reloading page'); } catch{}
        if (typeof location !== 'undefined' && location.reload) location.reload();
        return true;
      } catch (e) { try { console.warn('[XR][Reset] exception', e?.message||e); } catch{} return false; }
  }
  function refresh(){ const v=getViewer(); notify(getState(v)); return simSelection(); }
  function ensureDefaultActive(){
    const v=getViewer(); if(!v) return false;
    try {
      const running = (()=>{ try { return v?.getMetrics?.().running || null; } catch { return null; } })();
      if (!running) { v.stopSimulation?.(); v.startMDContinuous?.({}); notify(getState(v)); }
      return true;
    } catch { return false; }
  }
