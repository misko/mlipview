...
async function setupXR(
  engine,
  scene,
  { sessionMode = 'immersive-vr', referenceSpaceType = 'local-floor', optionalFeatures = [] } = {}
) {
  // Reuse helper only if same sessionMode already active
  if (
    window.vrHelper &&
    window.vrHelper.baseExperience &&
    window.vrHelper._sessionMode === sessionMode
  ) {
  ...
  try {
    ...
    if (mlip && typeof mlip.compute === 'function') {
      ...
      if (r && typeof r.then === 'function') {
        r.then((val) => {
          if (val && Number.isFinite(val.energy) && hud?.value)
            hud.value.text = val.energy.toFixed(3);
        }).catch(() => {});
...
export async function initVRApp() {
  return initXRLike({ sessionMode: 'immersive-vr' });
}
...
export async function initXRApp(opts) {
  return initXRLike(opts || {});
}
