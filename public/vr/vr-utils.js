...
export function computeMoleculeWorldBounds(scene, rootOrMasters) {
  ...
  if (Array.isArray(rootOrMasters)) {
    ...
    if (
      rootOrMasters.length === 1 &&
      rootOrMasters[0]?.name === 'molecule_root' &&
      rootOrMasters[0].getChildMeshes
    ) {
    ...
    rootOrMasters &&
    rootOrMasters.name === 'molecule_root' &&
    rootOrMasters.getChildMeshes
  ) {
...
export function refreshMoleculeMasters(scene, { force, molState } = {}) {
  ...
  try {
    ...
    if (explicitRoot) {
      try {
        if (typeof window !== 'undefined' && window.__XR_ROT_DEBUG)
          console.log(
            '[XRDBG][vr-utils] explicit molecule_root detected',
            explicitRoot.name,
            'type',
            explicitRoot.getClassName && explicitRoot.getClassName()
          );
      } catch {}
