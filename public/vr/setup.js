...
import { ensureWorldHUD as ensureXRHudBars, forceWorldHUD as forceXRHudBars } from './xr-hud-bars.js';
...
export function createVRSupport(scene, { picking } = {}) {
  ...
  function ensureXRHUD(){
    ...
    try {
      ...
      if(!scene || typeof BABYLON==='undefined' || !BABYLON.GUI || !BABYLON.GUI.AdvancedDynamicTexture){ console.warn('[XR][HUD] prerequisites missing'); return; }
      ensureXRHudBars({ scene, getViewer: ()=>{ try { return window._viewer || window.viewerApi; } catch{} return null; } });
  ...
  function forceWorldHUD(){ return forceXRHudBars({ scene, getViewer: ()=>{ try { return window._viewer || window.viewerApi; } catch{} return null; } }); }
