...
export function computeRadialDelta(state, inputs, config = {}) {
  ...
  let projection = 0;
  try {
    const baseDir = vecNorm(state.ctrlVec0);
    projection = vecDot(ctrlVec, baseDir) - vecDot(state.ctrlVec0, baseDir);
  } catch {}
  let forward = 0;
  try {
    const camForward = vecNorm(ctrlForward);
    forward = vecDot(ctrlVec, camForward) - vecDot(state.ctrlVec0, camForward);
  } catch {}
  let adaptive = 0;
  let ratio = 1;
  if (mode === 'adaptive') {
    const base = state.controllerDist0 || 1e-4;
    ratio = distCtrl / base;
    let frac = (ratio - 1) * adaptGain;
    if (frac > adaptMax) frac = adaptMax;
    else if (frac < -adaptMax) frac = -adaptMax;
    adaptive = frac * state.initialRadius;
  }
  let exp = 0;
  if (mode === 'exp') {
    const base = state.controllerDist0 || 1e-4;
    ratio = distCtrl / base;
    const f = Math.pow(ratio, expK) - 1;
    exp = f * expGain * state.initialRadius;
  }
  let deltaR;
