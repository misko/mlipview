...
export function createVRLaserManager({
  scene,
  xrHelper,
  controllerState,
  getControllerNode,
  ...
  const cfg = {
    baseDiam: (typeof window !== 'undefined' && window.vrLaserBaseDiam) || 0.01,
    hoverDiam: (typeof window !== 'undefined' && window.vrLaserHoverDiam) || 0.02,
  ...
  function ensureForController(c) {
    ...
    if (!st) {
      st = { pressed: false, lastYaw: 0, lastPitch: 0, downTime: 0, tapHandled: false };
      ...
      try {
        ...
        if (node) {
          const mesh = BABYLON.MeshBuilder.CreateCylinder(
            ...
            { height: cfg.maxLength, diameter: cfg.baseDiam, tessellation: 12, subdivisions: 1 },
            scene
          );
  ...
  function updateController(c) {
    ...
    if (typeof BABYLON !== 'undefined') {
      try {
        ...
        if (node?.getDirection && node.getAbsolutePosition) {
          ...
          if (res) {
            if (res.kind === 'atom') hoverKind = 'atom';
            else if (res.kind === 'bond') hoverKind = 'bond';
    ...
    if (mesh) {
      ...
      if (mat?.emissiveColor) {
        if (hoverKind) {
          mat.emissiveColor.r = cfg.hoverColor.r;
          mat.emissiveColor.g = cfg.hoverColor.g;
          mat.emissiveColor.b = cfg.hoverColor.b;
          mat.alpha = 1.0;
        } else {
          mat.emissiveColor.r = cfg.baseColor.r;
          mat.emissiveColor.g = cfg.baseColor.g;
          mat.emissiveColor.b = cfg.baseColor.b;
          mat.alpha = 0.95;
        }
      }
    }
  ...
  function getInfo() {
    ...
    try {
      const inputs =
        xrHelper && xrHelper.input && xrHelper.input.controllers ? xrHelper.input.controllers : [];
      for (const c of inputs) {
        const st = controllerState.get(c);
        if (!st) {
          out.push({ id: c?.uniqueId || 'unknown', hoverKind: null, diameter: undefined });
          continue;
        }
        out.push({
          id: c?.uniqueId || 'unknown',
          hoverKind: st.hoverKind || null,
          diameter: st.laserDiameterCurrent || null,
        });
      }
    } catch {}
    return out;
