  ...
  function updateDirection() {
    ...
    try {
      ...
      if (typeof cam.getFrontPosition === 'function' && cam.position) {
        ...
        if (front) {
          tmp.copyFrom(front).subtractInPlace(cam.position).normalize();
          fwd = tmp;
        }
      }
      if (!fwd && typeof cam.alpha === 'number' && typeof cam.beta === 'number') {
        ...
        tmp
          .set(Math.cos(cam.alpha) * sinB, Math.cos(cam.beta), Math.sin(cam.alpha) * sinB)
          .normalize();
        fwd = tmp;
      }
      if (fwd && head && head.direction) {
        head.direction.x = fwd.x;
        head.direction.y = fwd.y;
        head.direction.z = fwd.z;
      }
      if (head && head.position && cam.position) {
        head.position.x = cam.position.x;
        head.position.y = cam.position.y;
        head.position.z = cam.position.z;
      }
    } catch {}
  }
