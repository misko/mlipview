...
export function installTouchControls({ canvas, scene, camera, picking } = {}) {
  ...
  try {
    if (typeof window !== 'undefined' && window.__MLIPVIEW_NO_TOUCH) {
      console.warn('[touchControls] install aborted by __MLIPVIEW_NO_TOUCH');
      return;
    }
  } catch {}
  // Mark globally so other systems (like pickingService) can avoid double-attaching touch listeners
  try {
    if (typeof window !== 'undefined') window.__MLIPVIEW_TOUCH_INSTALLED = true;
  } catch {}
  ...
  const synth = (type) => {
    try {
      ...
      try {
        ev.__mlip_synthetic_from_touch = true;
      } catch {}
  ...
  const onMove = (e) => {
    ...
    if (mode === 'pinch' && touches.size >= 2) {
      ...
      if (!isDragging && camera && lastSingle) {
        ...
        if (DBG)
          console.log('[touchControls] rotate move', {
            dx,
            dy,
            alpha: camera.alpha,
            beta: camera.beta,
  ...
  const onEnd = (e) => {
    ...
    if (touches.size === 0) {
      if (DBG) console.log('[touchControls] gesture end');
      mode = 'none';
      lastSingle = null;
    }
    try {
    ...
    if (touchCapable && camera && camera.inputs) {
      ...
      if (isBrowser && window.__MLIPVIEW_FORCE_REMOVE_POINTER_INPUTS === true) {
        if (typeof camera.inputs.removeByType === 'function') {
          ...
          try {
            camera.inputs.removeByType('pointers');
          } catch {}
        }
        if (typeof camera.inputs.remove === 'function') {
          try {
            camera.inputs.remove(camera.inputs.attached && camera.inputs.attached.pointers);
          } catch {}
        }
      }
    }
