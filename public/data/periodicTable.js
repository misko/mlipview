...
function defaultColorForZ(z){
  ...
  function hslToRgb(h,s,l){
    ...
    let r=0,g=0,b=0; if(hp>=0&&hp<1){r=c;g=x;} else if(hp<2){r=x;g=c;} else if(hp<3){g=c;b=x;} else if(hp<4){g=x;b=c;} else if(hp<5){r=x;b=c;} else {r=c;b=x;}
    const m=l-c/2; return [r+m,g+m,b+m];
  }
  const [r,g,b]=hslToRgb(h,s,l); try { return new BABYLON.Color3(r,g,b); } catch { return { r, g, b, toHexString(){ return '#888'; } }; }
}
...
const OVERRIDES = {
  H:  { covRad: 0.31, vdw: 1.20, color: new BABYLON.Color3(0.95,0.97,1.00), scale: 0.45 },
  C:  { covRad: 0.76, vdw: 1.70, color: new BABYLON.Color3(0.22,0.24,0.28), scale: 0.70 },
  ...
  S:  { covRad: 1.05, vdw: 1.80, color: new BABYLON.Color3(0.98,0.85,0.27), scale: 0.80 },
  F:  { covRad: 0.57, vdw: 1.47, color: new BABYLON.Color3(0.56,0.93,0.56), scale: 0.60 },
  Cl: { covRad: 1.02, vdw: 1.75, color: new BABYLON.Color3(0.20,0.95,0.20), scale: 0.72 },
  P:  { covRad: 1.07, vdw: 1.80, color: new BABYLON.Color3(1.00,0.50,0.00), scale: 0.78 },
  Si: { covRad: 1.11, vdw: 2.10, color: new BABYLON.Color3(0.94,0.78,0.62), scale: 0.80 },
  Br: { covRad: 1.20, vdw: 1.85, color: new BABYLON.Color3(0.65,0.16,0.16), scale: 0.78 },
  I:  { covRad: 1.39, vdw: 1.98, color: new BABYLON.Color3(0.58,0.00,0.58), scale: 0.80 },
  Sn: { covRad: 1.39, vdw: 2.17, color: new BABYLON.Color3(0.66,0.66,0.72), scale: 0.82 },
  Fe: { covRad: 1.26, vdw: 1.94, color: new BABYLON.Color3(0.80,0.36,0.36), scale: 0.78 },
  Cu: { covRad: 1.32, vdw: 1.96, color: new BABYLON.Color3(0.72,0.45,0.20), scale: 0.80 },
  Zn: { covRad: 1.22, vdw: 2.01, color: new BABYLON.Color3(0.49,0.50,0.69), scale: 0.80 },
  Ag: { covRad: 1.45, vdw: 2.10, color: new BABYLON.Color3(0.75,0.75,0.80), scale: 0.84 },
  Au: { covRad: 1.36, vdw: 2.16, color: new BABYLON.Color3(0.85,0.68,0.13), scale: 0.84 },
  X:  { covRad: 0.80, vdw: 1.60, color: new BABYLON.Color3(0.70,0.70,0.70), scale: 0.70 }
};
...
export function getElementInfo(symbol){
  ...
  return { Z, symbol: sym, covRad: ovr.covRad ?? base.covRad, vdw: ovr.vdw ?? base.vdw, color: ovr.color ?? base.color, scale: ovr.scale ?? base.scale };
}

export const ELEMENTS_BY_SYMBOL = Object.fromEntries(SYMBOLS.map(sym => [sym, getElementInfo(sym)]));
