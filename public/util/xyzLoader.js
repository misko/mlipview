...
export function parseXYZ(text) {
  ...
  const toNum = (v)=>{ const t=String(v).replace(/[;,]$/,''); const n=parseFloat(t); return Number.isFinite(n)? n : NaN; };
  ...
  try {
    if (!tags.__abc && Number.isFinite(abcParts.a) && Number.isFinite(abcParts.b) && Number.isFinite(abcParts.c)) {
      tags.__abc = [abcParts.a, abcParts.b, abcParts.c];
    }
  } catch {}
  // If abc + angles provided, synthesize cell parameters
  try {
    if (!cell && (tags.__abc || tags.__angles || tags.__alpha!=null || tags.__beta!=null || tags.__gamma!=null)) {
  ...
  const gamma = (tags.__angles ? tags.__angles[2] : (tags.__gamma ?? 90));
      if ([a,b,c,alpha,beta,gamma].every(Number.isFinite)) {
        ...
        const cx=c*cb, cy=c*((ca - cb*cg)/sg), cz=c*Math.sqrt(Math.max(0,1 - cb*cb - ((ca - cb*cg)/sg)**2));
        cell = { a:{x:ax,y:ay,z:az}, b:{x:bx,y:by,z:bz}, c:{x:cx,y:cy,z:cz}, enabled:true, originOffset:{x:0,y:0,z:0} };
      }
    }
  } catch {}
  const elements = [];
...
export function applyXYZToState(molState, parsed) {
  ...
  if (parsed.temperature != null) {
    ...
    try { if (typeof window !== 'undefined') window.__MLIP_TARGET_TEMPERATURE = Number(parsed.temperature); } catch {}
    try { molState.dynamics = molState.dynamics || {}; molState.dynamics.targetTemperature = Number(parsed.temperature); } catch {}
  }
